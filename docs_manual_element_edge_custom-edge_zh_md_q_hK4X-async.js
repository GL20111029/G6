((typeof globalThis !== 'undefined' ? globalThis : self)["makoChunk_@antv/g6-site"] = (typeof globalThis !== 'undefined' ? globalThis : self)["makoChunk_@antv/g6-site"] || []).push([
        ['docs/manual/element/edge/custom-edge.zh.md?type=text'],
{ "docs/manual/element/edge/custom-edge.zh.md?type=text": function (module, exports, __mako_require__){
"use strict";
__mako_require__.d(exports, "__esModule", {
    value: true
});
__mako_require__.d(exports, "texts", {
    enumerable: true,
    get: function() {
        return texts;
    }
});
var _interop_require_wildcard = __mako_require__("@swc/helpers/_/_interop_require_wildcard");
var _reactrefresh = /*#__PURE__*/ _interop_require_wildcard._(__mako_require__("../../node_modules/.pnpm/react-refresh@0.14.2/node_modules/react-refresh/runtime.js"));
__mako_require__("docs/manual/element/edge/custom-edge.zh.md?watch=parent");
var prevRefreshReg;
var prevRefreshSig;
prevRefreshReg = self.$RefreshReg$;
prevRefreshSig = self.$RefreshSig$;
self.$RefreshReg$ = (type, id)=>{
    _reactrefresh.register(type, module.id + id);
};
self.$RefreshSig$ = _reactrefresh.createSignatureFunctionForTransform;
const texts = [
    {
        "value": "G6 提供了多种",
        "paraId": 0
    },
    {
        "value": "内置边",
        "paraId": 1
    },
    {
        "value": "类型，包含 ",
        "paraId": 0
    },
    {
        "value": "line（直线边）",
        "paraId": 2
    },
    {
        "value": "、",
        "paraId": 0
    },
    {
        "value": "polyline（折线边）",
        "paraId": 3
    },
    {
        "value": "、",
        "paraId": 0
    },
    {
        "value": "quadratic（二次贝塞尔曲线边）",
        "paraId": 4
    },
    {
        "value": "、",
        "paraId": 0
    },
    {
        "value": "cubic（三次贝塞尔曲线边）",
        "paraId": 5
    },
    {
        "value": "、",
        "paraId": 0
    },
    {
        "value": "cubic-horizontal（水平三次贝塞尔曲线边）",
        "paraId": 6
    },
    {
        "value": "、",
        "paraId": 0
    },
    {
        "value": "cubic-vertical（垂直三次贝塞尔曲线边）",
        "paraId": 7
    },
    {
        "value": " 等。这些内置边能够满足大部分基础场景需求。",
        "paraId": 0
    },
    {
        "value": "但在实际项目中，你可能会遇到这些基础边无法满足的需求。这时，你需要创建自定义边。别担心，这比你想象的要简单！",
        "paraId": 8
    },
    {
        "value": "在 G6 中，一条完整的边通常由以下几个部分组成：",
        "paraId": 9,
        "tocIndex": 0
    },
    {
        "value": "key",
        "paraId": 10,
        "tocIndex": 0
    },
    {
        "value": " ：边的主图形，表示边的主要形状，例如直线、折线等；",
        "paraId": 10,
        "tocIndex": 0
    },
    {
        "value": "label",
        "paraId": 10,
        "tocIndex": 0
    },
    {
        "value": " ：文本标签，通常用于展示边的名称或描述；",
        "paraId": 10,
        "tocIndex": 0
    },
    {
        "value": "arrow",
        "paraId": 10,
        "tocIndex": 0
    },
    {
        "value": " ：箭头，用于表示边的方向；",
        "paraId": 10,
        "tocIndex": 0
    },
    {
        "value": "halo",
        "paraId": 10,
        "tocIndex": 0
    },
    {
        "value": " ：主图形周围展示的光晕效果的图形。",
        "paraId": 10,
        "tocIndex": 0
    },
    {
        "value": "选择合适的方式",
        "paraId": 11,
        "tocIndex": 0
    },
    {
        "value": "创建自定义边的方式主要有两种途径：",
        "paraId": 12,
        "tocIndex": 1
    },
    {
        "value": "推荐",
        "paraId": 13,
        "tocIndex": 1
    },
    {
        "value": "这是最常用的方式，你可以选择继承以下类型之一：",
        "paraId": 14,
        "tocIndex": 2
    },
    {
        "value": "BaseEdge",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": " - 最基础的边类，提供边的核心功能",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": "Line",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": " - 直线边",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": "Polyline",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": " - 折线边",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": "Quadratic",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": " - 二次贝塞尔曲线边",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": "Cubic",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": " - 三次贝塞尔曲线边",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": "CubicVertical",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": " - 垂直三次贝塞尔曲线边",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": "CubicHorizontal",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": " - 水平三次贝塞尔曲线边",
        "paraId": 15,
        "tocIndex": 2
    },
    {
        "value": "为什么选择这种方式？",
        "paraId": 16,
        "tocIndex": 2
    },
    {
        "value": "📌 ",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "代码量少",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "：复用现有边的属性和方法，只需专注于新增功能",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "📌 ",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "开发迅速",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "：适合大多数项目需求，快速实现业务目标",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "📌 ",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "易于维护",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "：代码结构清晰，继承关系明确",
        "paraId": 17,
        "tocIndex": 2
    },
    {
        "value": "如果你选择继承现有边类型（推荐），可以直接跳到 ",
        "paraId": 18,
        "tocIndex": 2
    },
    {
        "value": "三步创建你的第一个自定义边",
        "paraId": 19,
        "tocIndex": 2
    },
    {
        "value": " 开始实践。大部分用户都会选择这种方式！",
        "paraId": 18,
        "tocIndex": 2
    },
    {
        "value": "高级用法",
        "paraId": 20,
        "tocIndex": 2
    },
    {
        "value": "如果现有边类型都不满足需求，你可以基于 G 的底层图形系统从零创建边。",
        "paraId": 21,
        "tocIndex": 3
    },
    {
        "value": "为什么选择这种方式？",
        "paraId": 22,
        "tocIndex": 3
    },
    {
        "value": "📌 ",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "最大自由度",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "：完全控制边的每个细节，实现任意复杂效果",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "📌 ",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "特殊需求",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "：现有边类型无法满足的高度定制场景",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "📌 ",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "性能优化",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "：针对特定场景的性能优化",
        "paraId": 23,
        "tocIndex": 3
    },
    {
        "value": "从零开发的自定义边需要自行处理所有细节，包括图形绘制、事件响应、状态变化等，开发难度较大。这里可以直接参考 ",
        "paraId": 24
    },
    {
        "value": "源码",
        "paraId": 24
    },
    {
        "value": " 进行实现。",
        "paraId": 24
    },
    {
        "value": "让我们从最基础的 ",
        "paraId": 25,
        "tocIndex": 4
    },
    {
        "value": "BaseEdge",
        "paraId": 25,
        "tocIndex": 4
    },
    {
        "value": " 开始，实现一个自定义直线边：",
        "paraId": 25,
        "tocIndex": 4
    },
    {
        "value": "import { Graph, register, BaseEdge, ExtensionCategory } from '@antv/g6';\n\nclass MyLineEdge extends BaseEdge {\n  getKeyStyle(attributes) {\n    return { ...super.getKeyStyle(attributes), lineWidth: 2, stroke: '#A4D3EE' };\n  }\n\n  getKeyPath(attributes) {\n    const { sourceNode, targetNode } = this;\n    const [x1, y1] = sourceNode.getPosition();\n    const [x2, y2] = targetNode.getPosition();\n\n    return [\n      ['M', x1, y1],\n      ['L', x2, y2],\n    ];\n  }\n}\n\nregister(ExtensionCategory.EDGE, 'my-line-edge', MyLineEdge);\n\nconst graph = new Graph({\n  container: 'container',\n  height: 200,\n  data: {\n    nodes: [\n      { id: 'node1', style: { x: 100, y: 50 } },\n      { id: 'node2', style: { x: 300, y: 120 } },\n    ],\n    edges: [{ source: 'node1', target: 'node2' }],\n  },\n  node: {\n    style: {\n      fill: '#7FFFD4',\n      stroke: '#5CACEE',\n      lineWidth: 2,\n    },\n  },\n  edge: {\n    type: 'my-line-edge',\n    style: {\n      zIndex: 3,\n    },\n  },\n});\n\ngraph.render();\n",
        "paraId": 26,
        "tocIndex": 4
    },
    {
        "value": "import { BaseEdge } from '@antv/g6';\nimport type { BaseEdgeStyleProps } from '@antv/g6';\n\nclass MyLineEdge extends BaseEdge {\n  // 定义边的样式，可以添加或覆盖默认样式\n  protected getKeyStyle(attributes: Required<BaseEdgeStyleProps>) {\n    // 调用父类方法获取基础样式，然后添加自定义样式\n    return { ...super.getKeyStyle(attributes), lineWidth: 2, stroke: '#A4D3EE' };\n  }\n\n  // 实现抽象方法：定义边的路径\n  // 这是 BaseEdge 的抽象方法，所有子类必须实现\n  protected getKeyPath(attributes) {\n    // 获取源节点和目标节点\n    const { sourceNode, targetNode } = this;\n\n    // 获取节点的位置坐标\n    const [x1, y1] = sourceNode.getPosition();\n    const [x2, y2] = targetNode.getPosition();\n\n    // 返回SVG路径数组，定义从起点到终点的直线\n    return [\n      ['M', x1, y1],\n      ['L', x2, y2],\n    ];\n  }\n}\n",
        "paraId": 27,
        "tocIndex": 5
    },
    {
        "value": "getKeyStyle",
        "paraId": 28
    },
    {
        "value": ": 定义边的基本样式，如线宽、颜色等",
        "paraId": 28
    },
    {
        "value": "getKeyPath",
        "paraId": 28
    },
    {
        "value": ": 是 ",
        "paraId": 28
    },
    {
        "value": "BaseEdge",
        "paraId": 28
    },
    {
        "value": " 中的抽象方法，",
        "paraId": 28
    },
    {
        "value": "必须实现",
        "paraId": 28
    },
    {
        "value": "，它定义了边的路径形状",
        "paraId": 28
    },
    {
        "value": "使用 ",
        "paraId": 29,
        "tocIndex": 6
    },
    {
        "value": "register",
        "paraId": 29,
        "tocIndex": 6
    },
    {
        "value": " 方法注册边类型，这样 G6 才能识别你的自定义边：",
        "paraId": 29,
        "tocIndex": 6
    },
    {
        "value": "import { ExtensionCategory } from '@antv/g6';\n\nregister(ExtensionCategory.EDGE, 'my-line-edge', MyLineEdge);\n",
        "paraId": 30,
        "tocIndex": 6
    },
    {
        "value": "register",
        "paraId": 31,
        "tocIndex": 6
    },
    {
        "value": " 方法需要三个参数：",
        "paraId": 31,
        "tocIndex": 6
    },
    {
        "value": "扩展类别：",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": "ExtensionCategory.EDGE",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": " 表示这是一个边类型",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": "类型名称：",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": "my-line-edge",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": " 是我们给这个自定义边起的名字，后续会在配置中使用",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": "类定义：",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": "MyLineEdge",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": " 是我们刚刚创建的边类",
        "paraId": 32,
        "tocIndex": 6
    },
    {
        "value": "在图的配置中，通过设置 ",
        "paraId": 33,
        "tocIndex": 7
    },
    {
        "value": "edge.type",
        "paraId": 33,
        "tocIndex": 7
    },
    {
        "value": " 来使用我们的自定义边：",
        "paraId": 33,
        "tocIndex": 7
    },
    {
        "value": "const graph = new Graph({\n  container: 'container',\n  data: {\n    nodes: [\n      { id: 'node1', style: { x: 100, y: 100 } },\n      { id: 'node2', style: { x: 300, y: 150 } },\n    ],\n    edges: [{ source: 'node1', target: 'node2' }],\n  },\n  node: {\n    style: {\n      fill: '#7FFFD4',\n      stroke: '#5CACEE',\n      lineWidth: 2,\n    },\n  },\n  edge: {\n    type: 'my-line-edge',\n    style: {\n      zIndex: 3,\n    },\n  },\n});\n\ngraph.render();\n",
        "paraId": 34,
        "tocIndex": 7
    },
    {
        "value": "🎉 恭喜！你已经创建了第一个自定义边。",
        "paraId": 35,
        "tocIndex": 7
    },
    {
        "value": "G6 的节点是由 ",
        "paraId": 36,
        "tocIndex": 9
    },
    {
        "value": "G 图形系统",
        "paraId": 36,
        "tocIndex": 9
    },
    {
        "value": " 提供的图形原子单元绘制而成。以下是常见图形元素及其用途：",
        "paraId": 36,
        "tocIndex": 9
    },
    {
        "value": "图形元素",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "类型",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "描述",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "圆形",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "circle",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "适合表示状态、头像、圆形按钮等。可以参考 SVG 的 ",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "<circle>",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": " 元素",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "椭圆",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "ellipse",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "与 circle 类似，但支持横纵轴不同的场景。可以参考 SVG 的 ",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "<ellipse>",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": " 元素",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "图片",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "image",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "用于展示图标、用户头像、LOGO 等。可以参考 SVG 的 ",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "<image>",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": " 元素",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "直线",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "line",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "用于装饰、辅助连接等。可以参考 SVG 的 ",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "<line>",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": " 元素",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "路径",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "path",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "支持复杂图形，如箭头、圆弧、曲线、贝塞尔路径等。路径中包含一组命令与参数，这些命令有不同的语义，",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "具体用法",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "多边形",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "polygon",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "支持自定义图形，如五角星、箭头。可以参考 SVG 的 ",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "<polygon>",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": " 元素",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "折线",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "polyline",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "多点折线，适合复杂的连线结构。可以参考 SVG 的 ",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "<polyline>",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": " 元素",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "矩形",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "rect",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "最常用图形，适合作为容器、卡片、按钮等基础结构。可以参考 SVG 的 ",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "<rect>",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": " 元素",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "文本",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "text",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "显示名称、描述、标签等内容。提供简单的单行/多行文本排版能力，单行支持水平对齐、字符间距；多行支持显式换行符以及自动换行，垂直对齐",
        "paraId": 37,
        "tocIndex": 9
    },
    {
        "value": "更多原子图形和详细的属性请参考 ",
        "paraId": 38,
        "tocIndex": 9
    },
    {
        "value": "元素 - 图形（可选）",
        "paraId": 39,
        "tocIndex": 9
    },
    {
        "value": "所有这些图形都可通过 ",
        "paraId": 40,
        "tocIndex": 9
    },
    {
        "value": "upsert()",
        "paraId": 40,
        "tocIndex": 9
    },
    {
        "value": " 动态创建或更新，并自动管理图形状态和生命周期。",
        "paraId": 40,
        "tocIndex": 9
    },
    {
        "value": "开始自定义元素之前，你需要了解 G6 元素基类中的一些重要属性和方法：",
        "paraId": 41,
        "tocIndex": 10
    },
    {
        "value": "属性",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "类型",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "描述",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "shapeMap",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "Record<string, DisplayObject>",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "当前元素下所有图形的映射表",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "animateMap",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "Record<string, IAnimation>",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "当前元素下所有动画的映射表",
        "paraId": 42,
        "tocIndex": 11
    },
    {
        "value": "upsert(name, Ctor, style, container, hooks)",
        "paraId": 43
    },
    {
        "value": "在创建自定义元素时，你会频繁用到 ",
        "paraId": 44,
        "tocIndex": 13
    },
    {
        "value": "upsert",
        "paraId": 44,
        "tocIndex": 13
    },
    {
        "value": " 方法。它是 \"update or insert\" 的缩写，负责添加或更新元素中的图形：",
        "paraId": 44,
        "tocIndex": 13
    },
    {
        "value": "upsert(key: string, Ctor: { new (...args: any[]): DisplayObject }, style: Record<string, any>, container: DisplayObject);\n",
        "paraId": 45,
        "tocIndex": 13
    },
    {
        "value": "参数",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "类型",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "描述",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "key",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "string",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "图形的 key，即 ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "shapeMap",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " 中对应的 key。内置的 key 包括 ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "'key'",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "'label'",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "'halo'",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "'icon'",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "'port'",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "'badge'",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " key 不应使用特殊符号，会基于该值转化为驼峰形式调用 ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "getXxxStyle",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " 和 ",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "drawXxxShape",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": " 方法（见",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "元素约定",
        "paraId": 47,
        "tocIndex": 13
    },
    {
        "value": "）",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "Ctor",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "{ new (...args: any[]): DisplayObject }",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "图形类",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "style",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "Record<string, any>",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "图形样式",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "container",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "DisplayObject",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "挂载图形的容器",
        "paraId": 46,
        "tocIndex": 13
    },
    {
        "value": "例如，插入一个固定位置的紫色圆形：",
        "paraId": 48,
        "tocIndex": 13
    },
    {
        "value": "this.upsert(\n  'element-key', // 元素的唯一标识\n  'circle', // 图形类型，如 'rect', 'circle' 等\n  { x: 100, y: 100, fill: '#a975f3' }, // 样式配置对象\n  container, // 父容器\n);\n",
        "paraId": 49,
        "tocIndex": 13
    },
    {
        "value": "为什么要使用 ",
        "paraId": 50,
        "tocIndex": 13
    },
    {
        "value": "upsert",
        "paraId": 50,
        "tocIndex": 13
    },
    {
        "value": " 而不直接通过 ",
        "paraId": 50,
        "tocIndex": 13
    },
    {
        "value": "container.appendChild()",
        "paraId": 50,
        "tocIndex": 13
    },
    {
        "value": " 创建图形？因为：",
        "paraId": 50,
        "tocIndex": 13
    },
    {
        "value": "性能更好",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "：当状态变化或数据更新时，会智能地复用已有图形，而不是删除再重建，大大提高了渲染性能",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "代码更简洁",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "：不需要手动判断元素是否存在",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "便于管理",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "：所有通过 ",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "upsert",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": " 创建的图形都会被记录在节点的 ",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "shapeMap",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": " 中，你可以通过 ",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "this.getShape(key)",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": " 轻松获取",
        "paraId": 51,
        "tocIndex": 13
    },
    {
        "value": "render(attributes, container)",
        "paraId": 43
    },
    {
        "value": "每个自定义边类都必须实现 ",
        "paraId": 52,
        "tocIndex": 14
    },
    {
        "value": "render(attributes, container)",
        "paraId": 52,
        "tocIndex": 14
    },
    {
        "value": " 方法，它定义了该边如何被“绘制”出来。你可以在这里使用各种原子图形，组合出你想要的结构。",
        "paraId": 52,
        "tocIndex": 14
    },
    {
        "value": "render(style: Record<string, any>, container: Group): void;\n",
        "paraId": 53,
        "tocIndex": 14
    },
    {
        "value": "参数",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "类型",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "描述",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "style",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "Record<string, any>",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "元素样式",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "container",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "Group",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "容器",
        "paraId": 54,
        "tocIndex": 14
    },
    {
        "value": "getShape(name)",
        "paraId": 43
    },
    {
        "value": "有时，你需要在创建后修改某个子图形的属性，或者让子图形之间有交互关联。这时，",
        "paraId": 55,
        "tocIndex": 15
    },
    {
        "value": "getShape",
        "paraId": 55,
        "tocIndex": 15
    },
    {
        "value": " 方法可以帮你获取之前通过 ",
        "paraId": 55,
        "tocIndex": 15
    },
    {
        "value": "upsert",
        "paraId": 55,
        "tocIndex": 15
    },
    {
        "value": " 创建的任何图形：",
        "paraId": 55,
        "tocIndex": 15
    },
    {
        "value": "⚠️ 注意",
        "paraId": 56,
        "tocIndex": 15
    },
    {
        "value": "：图形的顺序很重要，如果图形 B 依赖图形 A 的位置，必须确保 A 先创建",
        "paraId": 56,
        "tocIndex": 15
    },
    {
        "value": "使用约定属性",
        "paraId": 57,
        "tocIndex": 16
    },
    {
        "value": "目前约定的元素属性包括：",
        "paraId": 58,
        "tocIndex": 16
    },
    {
        "value": "通过 ",
        "paraId": 59,
        "tocIndex": 16
    },
    {
        "value": "this.getSize()",
        "paraId": 59,
        "tocIndex": 16
    },
    {
        "value": " 获取元素的尺寸",
        "paraId": 59,
        "tocIndex": 16
    },
    {
        "value": "通过 ",
        "paraId": 60,
        "tocIndex": 16
    },
    {
        "value": "const [sourcePoint, targetPoint] = this.getEndpoints(attributes, false)",
        "paraId": 60,
        "tocIndex": 16
    },
    {
        "value": " 获取边的起点和终点（简单模式 - 不考虑节点形状，直接返回节点中心点或最近连接桩中心˝位置）",
        "paraId": 60,
        "tocIndex": 16
    },
    {
        "value": "通过 ",
        "paraId": 61,
        "tocIndex": 16
    },
    {
        "value": "const [sourcePoint, targetPoint] = this.getEndpoints(attributes)",
        "paraId": 61,
        "tocIndex": 16
    },
    {
        "value": " 获取边的起点和终点（优化模式 - 默认为 true，考虑节点形状，返回节点边界上的连接点）",
        "paraId": 61,
        "tocIndex": 16
    },
    {
        "value": "采用 ",
        "paraId": 62,
        "tocIndex": 16
    },
    {
        "value": "getXxxStyle",
        "paraId": 62,
        "tocIndex": 16
    },
    {
        "value": " 和 ",
        "paraId": 62,
        "tocIndex": 16
    },
    {
        "value": "drawXxxShape",
        "paraId": 62,
        "tocIndex": 16
    },
    {
        "value": " 配对的方式进行图形绘制",
        "paraId": 62,
        "tocIndex": 16
    },
    {
        "value": "getXxxStyle",
        "paraId": 63,
        "tocIndex": 16
    },
    {
        "value": " 用于获取图形样式，",
        "paraId": 63,
        "tocIndex": 16
    },
    {
        "value": "drawXxxShape",
        "paraId": 63,
        "tocIndex": 16
    },
    {
        "value": " 用于绘制图形。通过该方式创建的图形支持自动执行动画。",
        "paraId": 63,
        "tocIndex": 16
    },
    {
        "value": "其中 ",
        "paraId": 64,
        "tocIndex": 16
    },
    {
        "value": "Xxx",
        "paraId": 64,
        "tocIndex": 16
    },
    {
        "value": " 是调用 ",
        "paraId": 64,
        "tocIndex": 16
    },
    {
        "value": "upsert",
        "paraId": 65,
        "tocIndex": 16
    },
    {
        "value": " 方法时传入的 key 的驼峰形式。",
        "paraId": 64,
        "tocIndex": 16
    },
    {
        "value": "可通过 ",
        "paraId": 66,
        "tocIndex": 16
    },
    {
        "value": "this.context",
        "paraId": 66,
        "tocIndex": 16
    },
    {
        "value": " 访问 Graph 上下文",
        "paraId": 66,
        "tocIndex": 16
    },
    {
        "value": "提供了以下生命周期钩子函数，你可以在自定义边中重写这些方法，在关键时刻执行特定逻辑：",
        "paraId": 67,
        "tocIndex": 17
    },
    {
        "value": "钩子函数",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "触发时机",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "典型用途",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "onCreate",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "当边创建后完成入场动画时",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "绑定交互事件、初始化边状态、添加外部监听器",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "onUpdate",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "当边更新后完成更新动画时",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "更新依赖数据、调整相关元素、触发联动效果",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "onDestroy",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "当边完成退场动画并销毁后",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "清理资源、移除外部监听器、执行销毁通知",
        "paraId": 68,
        "tocIndex": 17
    },
    {
        "value": "G6 元素设计中最强大的一点，是可以将 ",
        "paraId": 69,
        "tocIndex": 18
    },
    {
        "value": "“状态响应”",
        "paraId": 69,
        "tocIndex": 18
    },
    {
        "value": " 与 ",
        "paraId": 69,
        "tocIndex": 18
    },
    {
        "value": "“绘制逻辑”",
        "paraId": 69,
        "tocIndex": 18
    },
    {
        "value": " 分离。",
        "paraId": 69,
        "tocIndex": 18
    },
    {
        "value": "你可以在边配置中定义每种状态下的样式：",
        "paraId": 70,
        "tocIndex": 18
    },
    {
        "value": "edge: {\n  type: 'custom-edge',\n  style: { stroke: '#eee' },\n  state: {\n    selected: {\n      stroke: '#f00',\n    },\n    hover: {\n      lineWidth: 3,\n      stroke: '#1890ff',\n    },\n  },\n}\n",
        "paraId": 71,
        "tocIndex": 18
    },
    {
        "value": "切换状态的方法:",
        "paraId": 72,
        "tocIndex": 18
    },
    {
        "value": "graph.setElementState(edgeId, ['selected']);\n",
        "paraId": 73,
        "tocIndex": 18
    },
    {
        "value": "这个状态会传入到 ",
        "paraId": 74,
        "tocIndex": 18
    },
    {
        "value": "render()",
        "paraId": 74,
        "tocIndex": 18
    },
    {
        "value": " 方法的 ",
        "paraId": 74,
        "tocIndex": 18
    },
    {
        "value": "attributes",
        "paraId": 74,
        "tocIndex": 18
    },
    {
        "value": " 中，由内部系统合并后的结果自动应用在图形上。",
        "paraId": 74,
        "tocIndex": 18
    },
    {
        "value": "也可以根据状态自定义渲染逻辑：",
        "paraId": 75,
        "tocIndex": 18
    },
    {
        "value": "protected getKeyStyle(attributes: Required<BaseEdgeStyleProps>) {\n  const style = super.getKeyStyle(attributes);\n\n  // 根据状态调整样式\n  if (attributes.states?.includes('selected')) {\n    return {\n      ...style,\n      stroke: '#1890ff',\n      lineWidth: 2,\n      shadowColor: 'rgba(24,144,255,0.2)',\n      shadowBlur: 15,\n    };\n  }\n\n  return style;\n}\n",
        "paraId": 76,
        "tocIndex": 18
    },
    {
        "value": "import { Graph, register, BaseEdge, ExtensionCategory } from '@antv/g6';\n\nclass MyPolylineEdge extends BaseEdge {\n  getKeyPath(attributes) {\n    const [sourcePoint, targetPoint] = this.getEndpoints(attributes);\n\n    return [\n      ['M', sourcePoint[0], sourcePoint[1]],\n      ['L', targetPoint[0] / 2 + (1 / 2) * sourcePoint[0], sourcePoint[1]],\n      ['L', targetPoint[0] / 2 + (1 / 2) * sourcePoint[0], targetPoint[1]],\n      ['L', targetPoint[0], targetPoint[1]],\n    ];\n  }\n}\n\nregister(ExtensionCategory.EDGE, 'my-polyline-edge', MyPolylineEdge);\n\nconst graph = new Graph({\n  container: 'container',\n  height: 200,\n  data: {\n    nodes: [\n      { id: 'node-0', style: { x: 100, y: 50, ports: [{ key: 'right', placement: [1, 0.5] }] } },\n      { id: 'node-1', style: { x: 250, y: 150, ports: [{ key: 'left', placement: [0, 0.5] }] } },\n    ],\n    edges: [{ source: 'node-0', target: 'node-1' }],\n  },\n  edge: {\n    type: 'my-polyline-edge',\n    style: {\n      startArrow: true,\n      endArrow: true,\n      stroke: '#F6BD16',\n    },\n  },\n  behaviors: ['drag-element'],\n});\n\ngraph.render();\n",
        "paraId": 77,
        "tocIndex": 20
    },
    {
        "value": "import { Graph, Line, register, BaseEdge, ExtensionCategory, subStyleProps } from '@antv/g6';\n\nclass LabelEdge extends Line {\n  render(attributes, container) {\n    super.render(attributes);\n    this.drawEndLabel(attributes, container, 'start');\n    this.drawEndLabel(attributes, container, 'end');\n  }\n\n  drawEndLabel(attributes, container, type) {\n    const key = type === 'start' ? 'startLabel' : 'endLabel';\n    const [x, y] = this.getEndpoints(attributes)[type === 'start' ? 0 : 1];\n\n    const fontStyle = {\n      x,\n      y,\n      dx: type === 'start' ? 15 : -15,\n      fontSize: 16,\n      fill: 'gray',\n      textBaseline: 'middle',\n      textAlign: type,\n    };\n    const style = subStyleProps(attributes, key);\n    const text = style.text;\n    this.upsert(`label-${type}`, 'text', text ? { ...fontStyle, ...style } : false, container);\n  }\n}\n\nregister(ExtensionCategory.EDGE, 'extra-label-edge', LabelEdge);\n\nconst graph = new Graph({\n  container: 'container',\n  height: 200,\n  data: {\n    nodes: [\n      { id: 'node-0', style: { x: 100, y: 100 } },\n      { id: 'node-1', style: { x: 300, y: 100 } },\n    ],\n    edges: [{ source: 'node-0', target: 'node-1' }],\n  },\n  edge: {\n    type: 'extra-label-edge',\n    style: {\n      startArrow: true,\n      endArrow: true,\n      stroke: '#F6BD16',\n      startLabelText: 'start',\n      endLabelText: 'end',\n    },\n  },\n  behaviors: ['drag-element'],\n});\n\ngraph.render();\n",
        "paraId": 78,
        "tocIndex": 21
    }
];
if (prevRefreshReg) self.$RefreshReg$ = prevRefreshReg;
if (prevRefreshSig) self.$RefreshSig$ = prevRefreshSig;
function registerClassComponent(filename, moduleExports) {
    for(const key in moduleExports)try {
        if (key === "__esModule") continue;
        const exportValue = moduleExports[key];
        if (_reactrefresh.isLikelyComponentType(exportValue) && exportValue.prototype && exportValue.prototype.isReactComponent) _reactrefresh.register(exportValue, filename + " " + key);
    } catch (e) {}
}
function $RefreshIsReactComponentLike$(moduleExports) {
    if (_reactrefresh.isLikelyComponentType(moduleExports || moduleExports.default)) return true;
    for(var key in moduleExports)try {
        if (_reactrefresh.isLikelyComponentType(moduleExports[key])) return true;
    } catch (e) {}
    return false;
}
registerClassComponent(module.id, module.exports);
if ($RefreshIsReactComponentLike$(module.exports)) {
    module.meta.hot.accept();
    _reactrefresh.performReactRefresh();
}

},
 }]);
//# sourceMappingURL=docs_manual_element_edge_custom-edge_zh_md_q_hK4X-async.js.map